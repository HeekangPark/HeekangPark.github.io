...
확률적 경사 하강법으로 데이터 하나하나에 대한 그라디언트를 계산해 파라미터를 업데이트하게 되면 사실 항상 오차 함수가 작아지는 방향으로 움직이는 것을 장담할 수 없다. 하지만 높은 '확률'로 오차 함수가 작아지는 방향으로 움직일 것이다. 이 때문에 '확률적(stochastic)'이라는 단어로 이 방법을 나타내는 것이다.[^1]

[^1]: 흔히 확률적 경사 하강법의 '확률적'이라는 말을 '확률적'으로 데이터 중 일부를 선택해서 학습하는 것이라고 이해하는 경우가 있다. 글에서 밝혔다시피, **아니다**. 확률적 경사 하강법도 모든 데이터를 사용한다.

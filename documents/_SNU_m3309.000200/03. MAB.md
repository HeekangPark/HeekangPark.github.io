...
이 식에서 $[\text{(목표값)} - \text{(이전 추정값)}]$ 항은 목표값과 이전 추정값 사이의 오차(error)를 구하는 항이다. 위 식을 계속 적용해 나가면 목표값과 추정값 사이의 오차는 점점 줄어들게 된다(= 추정값과 목표값은 점점 유사해진다). 식이 한 번 수행될 때마다 추정값은 $\text{(스텝 크기)}$만큼 목표값에 다가가게 된다. 일반적으로 스텝 크기(step size)는 $\alpha\_t(a)$ 또는 그냥 간단히 $\alpha$라 표기한다. Sample-average Method는 스텝 크기로 $\frac{1}{n}$을 사용한다(= 매 시행마다 $\frac{1}{n}$씩 가까워진다).
...
&= H_{t}(a) + \alpha \bbox[{{ site.data.color.mathjaxHighlight }}]{\sum _{x} \pi_t (x)(q_* (x) - B_t) \frac{\partial \pi_t (x)}{\partial H_t (a)} \frac{1}{\pi_t (x)}}\\[0.5em]
\end{align}
$$

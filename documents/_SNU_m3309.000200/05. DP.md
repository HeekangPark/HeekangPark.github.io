...
<span class="indent"/><span class="indent"/><span class="keyword-highlight">until</span> $\Delta < \theta$ (단, $\theta$는 추정(estimation)의 정확도를 결정하는 작은 양의 실수)
...
{:.mt-1}
<span class="keyword-highlight">Return</span> $\pi(s) = \underset{a}{\operatorname{arg max}} \sum_{s'\,r} p(s',\,r\,\|\,s,\,a) [r + \gamma v_{\pi} (s')]$
...
이때까지 살펴본 DP(Policy Iteration, Value Iteration)의 문제점 중 하나는 매 반복마다 상태 전체에 대해 연산을 수행해야 한다는 것이다. 그 결과 상태 수가 적을 때는 괜찮겠지만 상태 수가 너무 많아지면 계산 비용이 너무 많이 나오게 된다. Asynchronous DP는 여기서 착안한 DP 기법이다. Asynchronous DP에서는 다른 상태들의 가치가 업데이트됐든 안 됐든 상관없이 매 반복에 오직 하나의 상태들의 가치를 업데이트한다.[^11] 물론 이 과정에서 몇몇 상태들의 가치는 다른 상태들의 가치가 한 번만 업데이트될 때 여러 번 업데이트되기도 한다. 단 $\pi\_{\*}$로의 수렴을 보장하려면 반드시 모든 상태들의 가치들이 최소 1번 이상은 업데이트되어야 한다. 대신 업데이트되는 순서는 상관이 없다.

[^11]: 이에 반해, 앞에서 배웠던 Policy Iteration, Value Iteration 등의 DP는 Synchronous DP라 한다.

예를 들어 Value Iteration에 Asynchronous DP를 적용하게 되면 각 단계 $k$에 Value Iteration 업데이트 식[^12]을 이용해 상태 $s\_k$ 하나만 업데이트한다. $0 \le \gamma < 1$이고, $\\{\,s\_k\,\\}$ 안에 모든 상태가 최소 한 번씩 다 등장했다면,[^13] $v\_{k}$는 점진적으로 $v\_{\*}$로 수렴한다.

[^12]: $v\_{k+1}(s) = \max\_a \sum_{s',\,r} p(s',\,r \,\|\,s,\,a)[r + \gamma v\_k (s')]$
[^13]: 등장 순서는 랜덤이어도 된다.
...
Evaluation 과정과 Improvement 과정 간의 이런 관계[^14]를 GPI(Generalized Policy Iteration)라 부른다. 거의 모든 RL 풀이법들이 GPI를 사용한다. 즉 거의 모든 RL 풀이법이 정책과 가치 함수를 사용하고, 정책은 가치 함수에 대해 최적화되고(Improvement 과정) 가치 함수는 정책으로부터 유도된다(Evaluation 과정)는 것이다. Evaluation 과정과 Improvement 과정이 모두 안정화되면(stabilize) 정책과 가치 함수가 모두 최적화된 것이다.[^15]

[^14]: 두 과정이 번갈아가며, 각각 $v\_k$와 $\pi\_k$를 만들어내는 관계
[^15]: 두 과정이 안정화되었다는 말은, 현재 정책에 대해 대해 만들어진 가치 함수에서 탐욕적인 선택을 한 것이 현재 정책과 같다는 말이고, Bellman optimality equation에 의해 정책과 가치 함수가 각각 최적화되었음이 보장된다.
...
DP에서 각 상태의 가치 추정값은 다음 상태의 가치 추정값을 기반으로 업데이트된다. 이런 식으로 어떤 추정값을 다른 추정값을 기반으로 업데이트하는 것을 Bootstrapping이라 한다. 사실 많은 RL 풀이법들이  Bootstrapping을 사용한다.[^16] DP처럼 환경에 대한 완벽하고 정확한 모델이 없어도 말이다.

[^16]: 물론 모든 RL 풀이법들이 다 Bootstrap을 사용하는 것은 아니다.

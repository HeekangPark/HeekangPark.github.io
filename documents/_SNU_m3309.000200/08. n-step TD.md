...
$$V(S_t) \leftarrow V(S_t) + \alpha [(\bbox[{{ site.data.color.mathjaxHighlight }}]{R_{t + 1} + \gamma R_{t+2} + \cdots + \gamma^{n-1} R_{t+n} + \gamma^n V(S_{t+n})}) - V(S_t)]$$

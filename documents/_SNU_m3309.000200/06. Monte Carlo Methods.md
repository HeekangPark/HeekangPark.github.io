...
<span class="indent"/><span class="indent"/><span class="indent"/><span class="keyword-highlight">If</span> $A\_t \neq \pi(S\_t)$:<span class="comment-highlight indent"># 만약 현재 $A\_t$가 탐욕적으로 뽑힌 값($\pi(S\_t)$)과 다르다면 $\pi(A\_t \,\|\,S\_t) = 0$이 되어 이후의 $\rho$가 모두 0이 된다. 이렇게 되면 더 이상 단계를 진행할 필요가 없어지므로 바로 다음 에피소드로 넘어간다.</span>

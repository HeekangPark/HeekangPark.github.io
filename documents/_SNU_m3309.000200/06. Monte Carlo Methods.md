...
<span class="indent"/>모든 $s \in \mathcal{S}$에 대해, Return($s$) = [] (빈 리스트)로 초기화
...
.pseudo-code-block .code .indent-4 {
    padding-left: 8em;
}
...
First-visit MC Method를 이용해 $v\_{\pi}$를 추정하려면, 우선 정책 $\pi$를 따르면서 여러 번의 시뮬레이션을 진행한다. 그리고 각 First-visit 상태에 대해 Return들을 평균한다.
...
[^10]: 모델이 있다면 (비록 실제와 다를 순 있어도) 각 상태에서 어떤 행동을 수행할 수 있는지, 행동의 결과 얼마만큼의 보상이 나오고 어느 상태로 전이하는지 등을 알 수 있다. 이를 이용하면 (DP에서 했던 것처럼) 탐욕적인 선택을 할 수 있다: $\pi(s) = \underset{a}{\operatorname{arg max}} \sum_{s'\,r} p(s',\,r\,\|\,s,\,a) [r + \gamma v_{\pi} (s')]$
...
이렇게 두 과정을 번갈아가며 진행하면, 환경에 대한 어떠한 배경 지식 없이도 최적 정책을 찾을 수 있다. 이때 Evaluation 과정과 Improvement 과정은 에피소드 단위로 진행된다(episode-by-episode). 각 에피소드가 끝나면, 관측된 Return을 이용해 가치 함수를 계산하고(Evaluation 과정), 해당 에피소드에서 방문한 모든 상태들에서의 정책이 업데이트된다(Improvement 과정).
...
<span class="indent"/>모든 $s \in \mathcal{S}$, $a \in \mathcal{A}$에 대해, Return($s$, $a$) = [] (빈 리스트)로 초기화
...
<span class="indent"/><span class="indent"/><span class="indent"/><span class="keyword-highlight">If not</span> $(S\_0,\,A\_0)$ appears in $S\_0,\,A\_0,\,S\_1,\,A\_1,\,\cdots,\,S\_{t+1},\,A\_{t-1}$:
...
위 식을 보면 알겠지만, Importance-sampling ratio는 오직 두 정책과 이들을 따를 때 나오는 상태-행동 시퀸스에만 종속적이고, MDP 환경에는 독립적이다.

행동 정책 $b$로 만들어진 데이터들의 Return을 평균내면 $\mathbb{E}[G_t\,\|\,S\_t = s] = v\_{b}(s)$, $\mathbb{E}[G_t\,\|\,S\_t = s,\,A\_t = a] = q\_{b}(s,\,a)$가 나온다. 이때 각 $G\_t$에 Importance-sampling ratio $\rho\_{t:\,T-1}$를 곱해주면 우리가 필요한 목표 정책 $\pi$를 적용했을 때의 가치 $v\_{\pi}(s)$, $q\_{\pi}(s,\,a)$를 구할 수 있다.

$$\array{
    \mathbb{E}[\rho_{t:\,T-1}G_t\,|\,S_t = s] = v_{\pi}(s)\\[0.5em]
    \mathbb{E}[\rho_{t:\,T-1}G_t\,|\,S_t = s,\,A_t = a] = q_{\pi}(s,\,a)\\[0.5em]
}$$

여기서 잠깐 몇 가지를 약속하자. 우선 에피소드 연속적인 시간 단계(time step)를 사용하자.[^22] 그리고 $T(t)$를 시간 $t$ 이후 최초로 맞는 에피소드 종료 시간(time of termination),[^23] $G\_t$를 시간 $t$부터 $T(t)$까지 받는 Return이라 하자. 여기에 다음과 같이 집합 $\mathcal{J}(s)$, $\mathcal{K}(s)$를 정의하자.[^24]

[^22]: 예를 들어 $t=100$에 한 에피소드가 끝났다면, 다음 에피소드는 $t=101$에서 시작한다고 하는 것이다(참고로 기존 방법에는 다시 $t=1$에서부터 시작한다고 봤다).
...
{:.no-guide-line}
- $\mathcal{J}(s)$ :
  - First-visit method의 경우 : 행동 정책 $b$로 만든 데이터에서, 상태 $s$가 최초방문된 모든 시간 단계들을 모은 집합
...
위 식의 기댓값을 계산하면 Off-policy Prediction을 수행할 수 있다. 이때 위 식에서 기댓값을 푸는 방법이 두 가지 있다. 첫 번째 방법은 단순 평균을 구하는 방법이다. 이를 **Ordinary Importance Sampling**이라 한다.

$$\array {
V(s) = \displaystyle\frac{\sum_{t\in\mathcal{J}(s)} \rho_{t:\,T(t)-1} G_t}{| \mathcal{K}(s,\,a) |}\\[0.5em]
Q(s,\,a) = \displaystyle\frac{\sum_{t\in\mathcal{K}(s,\,a)} \rho_{t:\,T(t)-1} G_t}{| \mathcal{J}(s) |}\\[0.5em]
}$$
...
Off-policy MC Method에서는 Ordinary Importance Sampling을 사용할 떄와 Weighted Importance Sampling을 사용할 때 Incremental Implementation을 적용하는 방법이 조금 다르다. 
...
$$\array{
    ...
    Q_{n+1} = V_n + \displaystyle\frac{1}{n}(W_{n+1}G_{n+1} - Q_n)\\[0.5em]
}$$
...
Weighted Importance Sampling을 사용하는 Off-policy Every-visit MC Prediction을 Incremental Implementation로 구현하면 다음과 같이 된다.

<div class="pseudo-code-block">
<p class="title">Off-policy Every-visit MC Prediction (Weighted Importance Sampling, Incremental Implementation)</p>
<div class="code" markdown="block">
...
<span class="indent"/>모든 $s \in \mathcal{S}$, $a \in \mathcal{A}$에 대해, $C(s,\,a) = 0$으로 초기화
...
<span class="indent"/><span class="indent"/>$b \leftarrow$ $\pi$에 대해 커버리지 가정이 성립하는 임의의 정책

<span class="indent"/><span class="indent"/>$b$를 따라 에피소드 생성 : $S\_0,\,A\_0,\,R\_1,\,S\_1,\,A\_1,\,R\_2,\,\cdots,\,S\_{T-1},\,A\_{T-1},\,R\_T$

<span class="indent"/><span class="indent"/>$G \leftarrow 0$

<span class="indent"/><span class="indent"/>$W \leftarrow 1$

<span class="indent"/><span class="indent"/><span class="keyword-highlight">Loop</span> for each step of episode, $t = T-1,\,T-2,\,\cdots,\,0$, <span class="keyword-highlight">while</span> $W \neq 0$:

<span class="indent"/><span class="indent"/><span class="indent"/>$G \leftarrow \gamma G + R\_{t+1}$

<span class="indent"/><span class="indent"/><span class="indent"/>$C(S\_t,\,A\_t) \leftarrow C(S\_t,\,A\_t) + W$

<span class="indent"/><span class="indent"/><span class="indent"/>$Q(S\_t,\,A\_t) \leftarrow Q(S\_t,\,A\_t) + \frac{W}{C(S\_t,\,A\_t)} [G - Q(S\_t,\,A\_t)]$

<span class="indent"/><span class="indent"/><span class="indent"/>$W \leftarrow W \times \frac{\pi(A\_t\,\|\,S\_t)}{b(A\_t\,\|\,S\_t)}$
...
<div class="pseudo-code-block">
<p class="title">Off-policy MC Control (Weighted Importance Sampling, Incremental Implementation)</p>
<div class="code" markdown="block">
...
<span class="indent"/>모든 $s \in \mathcal{S}$에 대해, $\pi(s) \leftarrow \underset{a}{\operatorname{arg max}} Q(s,\,a)$로 초기화
...
<span class="indent"/><span class="indent"/>$b \leftarrow$ $\varepsilon$-soft한 임의의 정책
...
<span class="indent"/><span class="indent"/><span class="indent"/>$\pi(S\_t) \leftarrow \underset{a}{\operatorname{arg max}} Q(S\_t,\,a)$ <span class="comment-highlight indent"># 만약 $Q(S\_t,\,a)$를 최대로 만드는 $a$가 여러 개이면 그 중 랜덤하게 고른다.</span>

<span class="indent"/><span class="indent"/><span class="indent"/><span class="keyword-highlight">If</span> $A\_t \neq \pi(S\_t)$:<span class="comment-highlight indent"># 만약 현재 $A\_t$가 탐욕적으로 뽑힌 값($\pi(S\_t)$)과 다르다면 $\pi(A\_t \,\|\,S\_t) = 0$이 되어 $\rho = 0$이 된다. 이렇게 되면 더 이상 단계를 진행할 필요가 없어지므로 바로 다음 에피소드로 넘어간다.</span>

<span class="indent"/><span class="indent"/><span class="indent"/><span class="indent"/><span class="keyword-highlight">break

<span class="indent"/><span class="indent"/><span class="indent"/>$W \leftarrow W \times \frac{1}{b(A\_t\,\|\,S\_t)}$<span class="comment-highlight indent"># 원래 분자는 $\pi(A\_t \,\|\,S\_t)$인데, $\pi$는 탐욕적으로 만들어진 정책이므로 항상 $\pi(A\_t \,\|\,S\_t) = 1$이다.</span>

</div>
</div>

참고로 이 방법은 잠재적인 문제가 있다. 만약 한 에피소드에서의 행동들이 대부분 탐욕적인 행동이라면 위 방법은 에피소드 끝에서만 학습하게 된다. 그러나 반대로 만약 대부분의 행동들이 비탐욕적인(nongreedy) 행동이라면 학습의 속도가 느릴 것이다. 만약 이 문제가 심각하다면, 다음 문서에서 다룰 TD 학습(Temporal-difference Learning)을 적용하는 것이 좋다.
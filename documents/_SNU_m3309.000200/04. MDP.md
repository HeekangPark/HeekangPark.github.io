...
[^2]: 이 글에서 시간 간격을 이산적으로(discrete) 놓은 이유는 논의를 단순화하기 위함이다. 당연히 RL에서는 이산적인 시간뿐만 아니라 연속적인 시간도 다룬다. 참고로 연속적인 시간에서의 RL 문제에도 이산적인 시간에서의 RL을 다룰 떄의 아이디어를 확장해 사용할 수 있는 경우가 많다.
...
$$\array {
v_* (s) = \displaystyle \max_{a \in \mathcal{A}(s)} q_{\pi_*} (s, \, a)\\[0.5em]
q_* (s,\,a) = \mathbb{E} \left[ R_{t+1} + \gamma v_{*}(S_{t+1}) | S_t = s, A_t = a \right]\\[0.5em]
}$$

## Bellman Optimality Equation

최적 가치 함수도 가치 함수의 일종이기에 [위 문단](#kramdown_bellman-equation--가치-함수의-재귀성)에서 살펴본 Bellman Equation을 적용할 수 있다. 이때 최적 관리 함수는 

$$\begin{align}
v_* (s)
&= \max_{a \in \mathcal{A}(s)} q_{\pi_*} (s, \, a)\\[0.5em]
&= \max_a \mathbb{E}_{\pi_*} [G_t | S_t = s,\, A_t = a]\\[0.5em]
&= \max_a \mathbb{E}_{\pi_*} [R_{t+1} + \gamma G_{t+1} | S_t = s,\, A_t = a]\\[0.5em]
...
위 두 식을 각각 Bellman (optimality) equation for $v\_{\*}$, Bellman (optimality) equation for $q\_{\*}$이라 한다.[^15] 위 식을 보면 알 수 있듯이, 최적 정책을 따를 때 어떤 상태에서의 가치는 그 상태에서 최고의 행동을 했을 때 도달하는 상태에서의 가치와 같다.[^16]

[^15]: 둘을 합쳐서 한 번에 Bellman optimality equation이라 부르기도 한다.
[^16]: 정확히는, 최고의 행동을 해 도달하는 상태에서의 가치에 Discount를 적용한 값과 해당 행동을 함으로서 받은 보상을 더한 값에, 해당 행동이 일어날 확률을 곱한 값과 같다.

Finite MDP에서는 Bellman optimality equation for $v\_{\*}$의 해가 유일하다. 사실 Bellman optimality equation은 연립 식으로, 만약 $n$개의 상태가 있다면 $n$개의 미지수를 가진 $n$개의 연립 식이 된다. 만약 Finite MDP의 Dynamics Function $p$가 알려져 있다면, 이론적으로는 비선형 연립방정식을 푸는 다양한 방법들을 이용해 Bellman optimality equation을 풀 수 있다.

만약 $v\_{\*}$를 안다면 최적 정책을 결정하는 것은 비교적 쉽다. 각 상태 $s$에 대해, Bellman optimality equation을 최대화하는 한 개 이상의 행동이 있다. 이런 동작들에 대해 0이 아닌 확률(nonzero probability)을 배정하는 정책은 모두 최적 정책이다. 예를 들어, 다음 문제를 보자.
...
이 게임판에서의 상태는 말이 있는 위치라 볼 수 있다. 이 문제의 Bellman optimality equation for $v\_{\*}$을 풀어, 다음과 같이 이 다음과 같다고 해 보자.

<div class="scrollable" markdown="block">

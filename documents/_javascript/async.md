...
이 때문에 일반적으로 비동기 자바스크립트 라이브러리에서는 `try...catch` 구문이 아닌 `if...else` 구문으로 예외처리를 진행한다. 예를 들어, Node.js에서는 콜백 함수의 첫번째 인자(argument)로 예외를 전달하고[^9], 실제 데이터는 두 번째 인자부터 전달하는 전통(convention)이 있다. 그래서 콜백 함수는 실행되자마자 자신을 호출한 함수(caller)가 예외를 전달했는지 여부를 `if...else` 구문을 통해 확인해, 만약 예외가 전달되었다면 적절한 처리를 해 주어야 한다.

[^9]: 만약 전달할 예외가 없다면 `null` 또는 `undefined`를 전달한다.
...
Promise는 쉽게 말해 값을 저장할 수 있는 상태 머신(state machine)으로, `new` 키워드를 이용해 생성 가능하다. Promise는 생성될 때 "실행 함수(executor)"라 불리는 함수 하나를 인자로 받는다. 만약 Promise 객체를 생성할 때 아무런 인자를 전달하지 않거나, 함수 이외의 값을 전달하면 `TypeError`가 발생한다.

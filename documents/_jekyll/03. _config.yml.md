...
이 설정값은 반드시 있어야만 하는 설정값은 아니지만, 많은 템플릿에서 사용하고 있다. 사이트의 url을 설정하는 값이다. 예를 들어, 본 문서의 주소인 `{{ site.url }}{{ page.url }}`에서 url은 `{{ site.url }}`를 의미한다. 일반적으로 아래 baseurl 설정값과 함께 쌍으로 사용된다.

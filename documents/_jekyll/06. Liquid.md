...
Liquid에서는 배열을 원하는 대로 초기화할 수 없고, 하위 속성을 마음대로 추가/삭제할 수 없다. 또한 map 필터로 할 수 있는 일들은 거의 대부분 `{% raw %}{% for %}{% endraw %}` 태그를 이용하여 똑같이 할 수 있다. 필자의 경우 그래서 map 필터를 잘 사용하지 않는다.
...
reverse 필터는 입력값 배열의 순서를 뒤집는 필터이다. 만약 입력값이 Array 타입이 아니라면 해당 값이 있는 크기 1짜리 Array 타입으로 간주하고 필터가 작동한다.[^18] 참고로 배열의 크기가 1이므로 필터가 적용돼도 출력은 동일하다.

[^18]: 다시말해 배열을 생성하기 위해 reverse 필터를 사용할 수 있다는 뜻이다. 하지만 배열을 만들기 위해 "뒤집는" 필터를 사용하는 것은 직관적이지 않고, Liquid 설명서에서 split 필터를 이용한 배열 생성을 거의 표준인 양 소개하기에 이 방법은 잘 사용되지 않는다.

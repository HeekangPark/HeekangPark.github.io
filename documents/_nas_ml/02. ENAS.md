...
- Controller는 각 decision block에서 다음 사항을 결정함
  - 연결할 이전 노드(들)
    - 바로 직전 노드가 아닌, 훨씬 이전의 노드도 선택할 수 있게 허용(skip connection)
    - 여러 개 선택 가능
    - ($k$번째 layer를 결정하는) 인덱스 $k$인 노드는 $1$ ~ $k-1$까지의 인덱스 중 선택할 수 있음 : $2^{k-1}$가지 경우의 수 가능
  - 해당 노드에서 사용할 연산(Computation Operation)의 종류
    - 다음 6가지 중 택1
      - 3×3 Convolution
      - 5×5 Convolution
...
- 한 decision block의 결정 결과는 CNN의 한 layer를 만듦
  - $L$층의 layer를 만드려면 $L$개의 노드가 필요함 : $6^L \times 2^{L(L-1)/2}$가지 경우의 수 가능
...
- Convolutional Cell 만들기
  - $B$개의 노드를 가지는 DAG를 만듦
    - 한 노드는 *셀 안에서* 지역적으로 일어나는 연산을 의미
    - 노드 1, 2 : Cell로 들어오는 입력값(이전 Cell의 출력값)을 그대로 출력
      - Controller는 이 두 셀에 대해서는 결정을 할 필요가 없다.
    - 나머지 $B-2$개 노드 : Controller가 다음 두 개의 결정을 해서 만듦
      - 현재 노드의 입력값으로 쓸 이전 노드 2개
      - 위에서 선택된 이전 노드 2개의 출력값에 적용할 연산 2개
        - 다음 5개 중 택2(중복 가능)
          - Identity
          ...
          - 3×3 Average Pooling
          - 3×3 Max Pooling
      - 샘플링된 이전 노드 2개에 샘플링된 연산 2개를 각각 적용한 후, 그 값을 더하여 출력
    - ex) $B=4$일 때의 예시 (논문 Fig. 05 참조)
      - Terminology
        - $h_i$ : 인덱스 $i$인 노드의 출력값
      - 노드 1, 2 : Controller는 결정을 할 필요가 없다.
        - $h\_1$, $h\_2$
      - 노드 3 : Controller는 인덱스 2, 2와 연산 5×5 Separable Convolution, Identity를 뽑는다.
        - $h\_3 = sep_conv_55(h\_2) + id(h\_2)$
      - 노드 4 : Controller는 인덱스 3, 1와 연산 3×3 Average Pooling, 3×3 Separable Convolution을 뽑는다.
        - $h\_4 = avg_pool_33(h\_3) + sep_conv_33(h\_1)$
      - Cell의 출력값은 Loose End(다른 노드들의 입력으로 선택되지 않은 노드들)들을 Depth Dimension 방향으로 concatenate하여 구한다.

...
필자가 제일 처음 시도한 방식이 이 방식이었다. 처음에는 TeamViewer를 사용하다가 나중에는 Chrome Remote Desktop을 사용했는데, 사용해보니 이 방법에는 단점이 많았다.
...
5. 몇몇 회사의 경우 보안상의 이유로 RDP를 막아놓는 경우가 있어, 이런 경우에는 사용할 수 없다.

그래서 현재 이 방식은 안정적인 Wi-Fi 연결이 보장된 환경에서 가끔 사용하고 있다.
...
2. 어쨌든 commit, push, pull 등 추가적인 조작이 필요하다. 이 방법을 쓰다 보면 가끔 PC에서 commit/push 하는 것을 잊어버려 코딩을 할 수 없거나, iPad에서 push를 한 후 PC에서 pull하는 것을 잊어 충돌이 발생하거나 한다.
3. 만약 Git 이력 관리가 중요하다면 이 방법은 사용하기 어렵다. 예를 들어, PC에서 코딩하다가 갑자기 어딜 가야 해 iPad로 연속해 작업을 해야 하는 상황을 생각해 보자. 이 경우 PC에서 작업이 얼만큼 됐던 간에 일단 commit 후 push를 하고 이를 iPad에서 pull하는 작업이 필요하다. 개인 레포지토리에서야 이런 식으로 해도 상관이 없겠지만, 공동 레포지토리에서 iPad와의 동기화를 위해서 commit을 남발할 순 없으니 이 방법을 사용할 순 없다.
...
원하는 곳에서 바로 Code-Server를 실행할 수 있도록 다음과 같이 Code-Server 이미지를 실행하는 `run-code-server` 스크립트를 만들었다.
...
이 스크립트에 실행 권한을 주고, Code-Server의 설정이 담길 디렉토리를 만든다.
...
이제 Code-Server가 실행됐으면 하는 디렉토리로 이동해 위 스크립트를 실행한다. 참고로 이를 편하게 하기 위해 필자는 `run-code-server`가 있는 디렉토리를 `$PATH`에 등록해 놨다. 이렇게 하면 어디서든 `run-code-server`라 입력해도 위 스크립트를 바로 실행할 수 있다.

{% highlight bash %}
$ run-code-server
{% endhighlight %}

실행 중인 Code-Server를 종료하고 싶다면 그냥 `Ctrl + C`를 누르면 된다.

### 사용법

이제 iPad에서 다음 URL에 접속하면 웹 상에서 VSCode 환경을 띄우고 그 안에서 코딩을 할 수 있다.
...
이렇게 완벽해 보이는 Code-Server도 한 가지 문제점이 있다. 현재 한글을 입력하면 더듬더듬 입력되는 현상이 관찰되었다(예를 들어 "사랑"을 입력하면 "ㅅ사라랑랑" 이런 식으로 입력된다). Github Issue를 참조하니 이 문제는 Code-Server의 문제가 아닌, VSCode가 가지고 있는 문제라 한다. 이것이 패치되려면 최소 다음 VSCode 패치가 나올 때까지 기다려야 할 것 같다.
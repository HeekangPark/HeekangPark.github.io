...
{:.no-guide-line}
- 임의의 원소 $\mathbf{u} \in V$, 스칼라 $c \in \mathbb{R}$에 대해, $0\mathbf{u} = \mathbf{0}$
- 임의의 원소 $\mathbf{u} \in V$, 스칼라 $c \in \mathbb{R}$에 대해, $c\mathbf{0} = \mathbf{0}$
- 임의의 원소 $\mathbf{u} \in V$, 스칼라 $c \in \mathbb{R}$에 대해, $-\mathbf{u} = (-1)\mathbf{u}$
...
우리가 통상적으로 생각하는 벡터("여러 개의 값을 하나의 순서 있는 덩어리로 묶어놓은 것")는 벡터의 한 가지 종류일 뿐이다. 이외에도 다양한 종류의 벡터들이 있을 수 있다. 예를 들면, 모든 3×3 행렬들의 집합은 벡터공간이고(임의의 3×3 행렬들에 대해 위 10가지 공리가 모두 성립한다), 이 집합의 한 원소(3×3 행렬)는 벡터이다.
...
$$H = \text{Span}\{\mathbf{v}_1,\,\mathbf{v}_2,\,\cdots,\,\mathbf{v}_p\}$$

역으로, $H$가 벡터공간 $V$의 subspace라면, 위 식을 만족시키는 벡터공간 $V$의 원소 $\mathbf{v}\_1,\,\mathbf{v}\_2,\,\cdots,\,\mathbf{v}\_p \in V$가 반드시 존재한다. 이때, 집합 $\\{\mathbf{v}\_1,\,\mathbf{v}\_2,\,\cdots,\,\mathbf{v}\_p \\}$을 $H$의 **spanning set** 또는 **generating set**이라 부른다.
...
0\\[0.5em]
1\\[0.5em]
\end{bmatrix} = x_2 \mathbf{u} + x_4 \mathbf{v} + x_5 \mathbf{w}$$

{:.text-align-center}
(단, $\mathbf{u} = \begin{bmatrix} 2\\\\[0.5em]1\\\\[0.5em]0\\\\[0.5em]0\\\\[0.5em]0\\\\[0.5em]\end{bmatrix},\,\mathbf{v} = \begin{bmatrix} 1\\\\[0.5em]0\\\\[0.5em]-2\\\\[0.5em]1\\\\[0.5em]0\\\\[0.5em]\end{bmatrix},\,\mathbf{w} = \begin{bmatrix} -3\\\\[0.5em]0\\\\[0.5em]2\\\\[0.5em]0\\\\[0.5em]1\\\\[0.5em]\end{bmatrix}$)

따라서, 집합 $\\{\mathbf{u},\,\mathbf{v},\,\mathbf{w}\\}$은 $\text{Nul}\,A$의 spanning set이 된다.

$$\text{Nul}\,A = \text{Span}\{\mathbf{u},\,\mathbf{v},\,\mathbf{w}\}$$
...
위의 null space를 구할 때와 다르게, column space의 spanning set은 $A$의 열들 그 자체이다.

$$\text{Col}\,A = \text{Span}\{\begin{bmatrix}
-3\\[0.5em]
1\\[0.5em]
2\\[0.5em]
\end{bmatrix},\,\begin{bmatrix}

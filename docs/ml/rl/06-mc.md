...
가 된다. 따라서, Policy Improvement Theorem에 의해 $\pi' \ge \pi$이다(= $v_{\pi'}(s) \ge v_{\pi}(s)$).
...
::: details Code : On-policy first-visit MC control($\varepsilon$-soft policy 사용)
...
[^26]: 상태 $s$를 최초방문하는 에피소드 하나만 보고 First-visit method로 $V(s)$를 추정하는 극단적인 상황을 생각해 보자. Weighted Importance Sampling에서는 Importance-sampling ratio $\rho$가 약분되어 $V(s) = G_t$가 된다(아래 첨자는 생략했다). 이때 $G_t$는 $v_{\pi}(s)$가 아닌 $v_{b}(s)$를 나태내는 값이다. 즉 Weighted Importance Sampling은 편향되어 있다(biased). 하지만 Ordinary Importance Sampling에서는 $\|\mathcal{J}(s)\| = 1$이므로 $V(s) = \rho G_t$가 된다. 이는 $v_{\pi}(s)$를 나태내는 값이므로 Ordinary Importance Sampling은 편향되어 있지 않다(unbiased). 그러나 에피소드의 수가 많아질수록 편향(bias)는 점점 0으로 수렴하게 된다.<br/>한편 Importance-sampling ratio $\rho$는 그 크기의 상한, 하한이 없으므로 값이 매우 클 수 있다. 만약 $\rho=10$이라면(즉, 현재 보고 있는 에피소드의 상태-행동 시퀸스가 행동 정책을 따를 때보다 목표 정책을 따를 때 10배 더 잘 등장한다면) Ordinary Importance Sampling의 추정값은 실제 관측된 Return 값($G_t$)보다 10배 더 큰 값이 된다. 즉, Ordinary Importance Sampling은 분산이 크다. 비록 현재 보고있는 상태-행동 시퀸스가 목표 정책을 매우 잘 표현하고(representative) 있긴 하지만 말이다. 그러나 Weighted Importance Sampling의 경우 나올 수 있는 값이 최대 1이므로, 분산이 작다.
[^27]: 샘플의 수가 많아지면 편향이 작아진다.

#### Incremental Implementation

우리는 [k-armed Bandit Problem](/ml/rl/03-mab)에서 [Sample-average Method](/ml/rl/03-mab#sample-average-method)에 [Incremental Implementation](/ml/rl/03-mab#incremental-implementation)을 적용하는 것을 보았다. 보상을 평균내는 Sample-average Method와 유사하게 MC method에서는 Return을 평균내므로, Incremental Implementation을 적용할 수 있다.

On-policy MC Method에서는 Sample-average Method에서 했던 것과 정확히 같은 방법으로 Incremental Implementation을 적용하면 된다. 즉, $n$개의 에피소드를 보고 추정한 상태-가치 함수를 $V_n$, 행동-가치 함수를 $Q_n$, $n+1$번째 에피소드에서의 Return이 $G_{n+1}$이라 하면, 다음 식을 이용해 Incremental Implementation을 수행할 수 있다.
...
Ordinary Importance Sampling에서는 Return에 Importance-sampling ratio $\rho_{t:\,T(t)-1}$을 곱해 사용한다. 따라서 Return에 이를 곱해주면 Incremental Implementation을 수행할 수 있다. 즉, $n$개의 에피소드를 보고 추정한 상태-가치 함수를 $V_{n}$, 행동-가치 함수를 $Q_{n}$, $n+1$번째 에피소드에서의 Return을 $G_{n+1}$, 그리고 이에 상응하는 Importance-sampling ratio를 $W_{n+1}$[^28]이라 할 때, 다음 식을 이용해 Incremental Implementation을 수행할 수 있다.

[^28]: 즉 $W_{n} = \rho_{t_n :\, T(t_n) - 1}$이 된다. (단, $t_i$는 $\mathcal{J}(s)$의 $i$번째 원소)
...
Weighted Importance Sampling에서는 가중 평균을 계산하므로, 가중치(Importance-sampling ratio)들의 합을 저장하는 변수와 상태-가치 함수의 값 두 개를 각각 추적해야 한다. 즉, $n$개의 에피소드를 보고 추정한 상태-가치 함수를 $V_{n}$, 행동-가치 함수를 $Q_{n}$, $n$번째 에피소드에서의 Return을 $G_{n}$, $G_{n}$에 상응하는 Importance-sampling ratio를 $W_{n}$[^28], $C_n = \sum_{k=1}^{n} W_k$라 할 때[^29], 다음 식을 이용해 Incremental Implementation을 수행할 수 있다.
...
::: info Pseudo Code : Off-policy Every-visit MC Prediction (Weighted Importance Sampling, Incremental Implementation)
...
2\. Off-policy MC Prediction {.mt-1}

<span class="indent-1"/><span class="keyword-highlight">Loop</span> forever (for each episode):

<span class="indent-2"/>$b \leftarrow$ $\pi$에 대해 커버리지 가정이 성립하는 임의의 정책

<span class="indent-2"/>$b$를 따라 에피소드 생성 : $S_0,\,A_0,\,R_1,\,S_1,\,A_1,\,R_2,\,\cdots,\,S_{T-1},\,A_{T-1},\,R_T$
...
<span class="indent-3"/>$C(S_t,\,A_t) \leftarrow C(S_t,\,A_t) + W$

<span class="indent-3"/>$Q(S_t,\,A_t) \leftarrow Q(S_t,\,A_t) + \frac{W}{C(S_t,\,A_t)} [G - Q(S_t,\,A_t)]$

<span class="indent-3"/>$W \leftarrow W \times \frac{\pi(A_t\,\|\,S_t)}{b(A_t\,\|\,S_t)}$
...
::: info Pseudo-code : Off-policy MC Control (Weighted Importance Sampling, Incremental Implementation)
...
<span class="indent-3"/>$\pi(S_t) \leftarrow \underset{a}{\operatorname{arg max}} Q(S_t,\,a)$ <span class="comment-highlight"># 만약 $Q(S_t,\,a)$를 최대로 만드는 $a$가 여러 개이면 그 중 랜덤하게 고른다.</span>

<span class="indent-3"/><span class="keyword-highlight">If</span> $A_t \neq \pi(S_t)$: <span class="comment-highlight"># 만약 현재 $A_t$가 탐욕적으로 뽑힌 값($\pi(S_t)$)과 다르다면 $\pi(A_t \,\|\,S_t) = 0$이 되어 이후의 $\rho$가 모두 0이 된다. 이렇게 되면 더 이상 단계를 진행할 필요가 없어지므로 바로 다음 에피소드로 넘어간다.</span>

<span class="indent-4"/><span class="keyword-highlight">break</span>

<span class="indent-3"/>$W \leftarrow W \times \frac{1}{b(A_t\,\|\,S_t)}$ <span class="comment-highlight"># 원래 분자는 $\pi(A_t \,\|\,S_t)$인데, $\pi$는 탐욕적으로 만들어진 정책이므로 항상 $\pi(A_t \,\|\,S_t) = 1$이다.</span>

</div>

:::

참고로 이 방법은 잠재적인 문제가 있다. 만약 한 에피소드에서의 행동들이 대부분 탐욕적인 행동이라면 위 방법은 에피소드 끝에서만 학습하게 된다. 그러나 반대로 만약 대부분의 행동들이 비탐욕적인(nongreedy) 행동이라면 학습의 속도가 느릴 것이다. 만약 이 문제가 심각하다면 [TD Method](/ml/rl/07-td)을 적용하는 것이 좋다.

#### 구현하기

Off-policy MC Control을 이용해 블랙잭 게임에서 최적 정책을 찾아보자.

::: details Code : Off-policy MC Control (Incremental Implementation)
...
<v-image src="06-off-policy-mc-control-ordinary-importance-sampling.png" title="Fig.05 Ordinary Importance Sampling을 사용하는 Off-policy MC Control" description="500,000번 시뮬레이션 진행한 결과를 이용해 Ordinary Importance Sampling을 사용하는 Off-policy MC Control을 수행한 모습" />

<v-image src="06-off-policy-mc-control-weighted-importance-sampling.png" title="Fig.06 Weighted Importance Sampling을 사용하는 Off-policy MC Control" description="500,000번 시뮬레이션 진행한 결과를 이용해 Weighted Importance Sampling을 사용하는 Off-policy MC Control을 수행한 모습" />
